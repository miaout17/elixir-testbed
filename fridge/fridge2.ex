...
object Fridge
  ...
  def constructor(list)
    ...
    pid = Pid.spawn Fridge::Proto, 'start, [list]
  ...
  def start(foodlist)
    update(foodlist).loop
  end

  def update(foodlist)
    self.set_ivar('foodlist, foodlist).loop
  end
  ...
  def loop
    ...
    match {from, {'store, food}}
      from <<- 'ok
      update([food|@foodlist]).loop
    match {from, {'take, food}}
      if @foodlist.include?(food)
        from <<- {'ok, food}
        update(@foodlist.delete(food)).loop
      else
        from <<- 'not_found
        loop
      end
